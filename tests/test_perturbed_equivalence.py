...
from gameworld.envs import create_gameworld_env as create_env
from gameworld.perturbed_envs import create_gameworld_env as create_perturbed_env
...
def test_reset_equivalence(env_name):
    # --------------------------------------------------
    ...
    plt.imshow(obs1)
    plt.savefig(f"original_env_{env_name}.png")
    ...
    plt.imshow(obs2)
    plt.savefig(f"perturbed_env_{env_name}.png")

    plt.imshow(obs1 - obs2)
    plt.savefig(f"diff_env_{env_name}.png")
...
@pytest.mark.parametrize("env_name", ENV_NAMES)
def test_step_equivalence(env_name):
    # --------------------------------------------------
    # 1) seed BEFORE reset
